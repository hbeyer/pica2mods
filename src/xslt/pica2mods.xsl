  ...
  <xsl:template match="record">
    ...
    <mods:mods>
      ...
      <xsl:choose>
        ...
        <!-- Das folgende when war in der Vorgängerversion leer, was zum Auslassen des Titels bei Af-Sätzen führte. -->
        <xsl:when test="$type = 'f' and not(datafield[@tag = '021A']) and datafield[@tag = '036D']">
          <xsl:copy-of select="mods:titleInfo(datafield[@tag = '036D'], ())"/>
        </xsl:when>
    ...
    <mods:name type="{if (starts-with($field/subfield[@code = 'M'], 'Tb')) then 'corporate' else 'personal'}">
      ...
      <xsl:if test="$field/subfield[@code = ('a', 'A')]">
        <mods:namePart type="family">
          <xsl:value-of select="$field/subfield[@code = ('a', 'A')][1]"/>
        </mods:namePart>
      </xsl:if>
      <xsl:if test="$field/subfield[@code = ('d', 'D')]">
        <mods:namePart type="given">
          <xsl:value-of select="$field/subfield[@code = ('d', 'D')][1]"/>
        </mods:namePart>
      </xsl:if>
      ...
      <mods:displayForm>
        <xsl:choose>
          ...
          <xsl:when test="$field/subfield[@code = ('a', 'A') or @code = ('d', 'D')]">
            <xsl:value-of select="($field/subfield[@code = ('a', 'A')][1], $field/subfield[@code = ('d', 'D')][1])"
                          separator=", "/>
          </xsl:when>
